    ...
    def train(self):
        ...
        for i, params in enumerate(tqdm(param_set)):
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                # 실습: 3. Early stopping한 모델의 최종 iterations를 파라미터로 저장
                mlflow.log_params(
                    {"iterations": cls.best_iteration_}
                )
                # 실습: 4. self._parse_score_dict를 이용해 검증셋에 대한 스코어를 메트릭으로 저장
                mlflow.log_metrics(
                    self._parse_score_dict(
                        cls.get_best_score().get("validation")
                    )
                )
                # 실습: 5. signature를 포함하여 모델 정보 로깅
